    ...
    <TouchableOpacity onPress={onPress} style={[styles.item]}>
      <View style={styles.listItemInfo}>
        ...
        <View style={styles.titleAndAddressView}>
          ...
          <Text style={styles3.eventButtonDate}>
            {moment(item.datetime).locale('es').format('llll')}
          </Text>
        </View>
      </View>
    </TouchableOpacity>
...
export default function EventList({ navigation, route }) {
  ...
  const eventsOfType = useMemo(
    () =>
      eventsList?.filter((individualEvent) => {
        ...
        <View style={{ flex: 1, alignItems: 'center', marginVertical: 30 }}>
          ...
          {eventsOfType && eventsOfType.length ? (
            <>
              ...
              />
              <Text style={styles3.kindabrown}>
                Total de eventos: {eventsOfType.length}
              </Text>
            </>
          ) : (
            <Text style={styles.noInternetContainer}>
              No hay eventos de {cat} disponibles.
            </Text>
          )}
        </View>
      </View>
